...
namespace ConstructionLine.CodingChallenge.Tests
{
    public class SearchEngineTestsBase
    {
            ...
            foreach (var size in Size.All)
            {
                ...
                var expectedSizeCount = shirts
                    .Count(s => s.Size.Id == size.Id
                                && (searchOptions.Colors.Count == 0 || searchOptions.Colors.Select(c => c.Id).Contains(s.Color.Id)));
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = shirts
                    .Count(c => c.Color.Id == color.Id
                                && (searchOptions.Sizes.Count == 0 || searchOptions.Sizes.Select(s => s.Id).Contains(c.Size.Id)));
        ...
        protected static void AssertSpeedIsAcceptable(long elapsedTime)
        {
            var reasonableLengthInMs = 100;
            Assert.That(elapsedTime, Is.LessThan(reasonableLengthInMs), $"A reasonable implementation should not take more than {reasonableLengthInMs} ms to return the results.");
        }
    }
}